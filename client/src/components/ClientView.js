// Componente para vista cliente con solo 3 productos (imágenes) y botón verde
...
const ClientView = () => {
  ...
  const [loading, setLoading] = useState(true);  useEffect(() => {
    // Preload imágenes específicas para asegurar carga (incluyendo img2)
    const preloadImages = () => {
      const urls = [
    ...
    preloadImages();  // Preload antes de fetch// Fetch productos de backend y filtra solo 3 primeros con imágenes
axios.get('http://localhost:5000/api/products')
  ...
  .catch(err => {
    ...
    setLoading(false);
  });  }, []);  if (loading) return <p className="text-center mt-5">Cargando productos...</p>;  return (
    <section id="cliente-view" className="hero container mt-5">
